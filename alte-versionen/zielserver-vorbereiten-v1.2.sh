...
# - telepraxis-app wird nun pro Benutzer als /var/www/html/telepraxis-app-<benutzer>.php abgelegt
...
ensure_dirs() {
  ...
  install -d -o "$user" -g "$user" -m 0750 "$base"
...
text = pathlib.Path(app_file).read_text(encoding='utf-8')
if '/srv/telepraxis/inbox' not in text:
    raise SystemExit('Patch fehlgeschlagen: /srv/telepraxis/inbox nicht gefunden')
text = text.replace('/srv/telepraxis/inbox', inbox_dir)
pathlib.Path(app_file).write_text(text, encoding='utf-8')
PY
...
main() {
  ...
  app_target="${WEBROOT_DIR}/telepraxis-app-${target_user}.php"
